...
function queryOf(text: string): string {
  return text
    ...
    .replace(
      /(الساعة\s*(كام|كم)?|تاريخ\s*(النهارده|النهاردة|اليوم)|إيه\s*تاريخ|ايه\s*تاريخ|بالظبط|بالضبط|احنا\s*في\s*(شهر|سنة|يوم)?|كم\s*الساعة|و?التاريخ\b|what\s*time|today'?s?\s*date)/gu,
      " ",
    )
    .replace(/(^|\s)(و?إيه|و?ايه|و?ما هي|و?ما هو|هل|من فضلك|كمان|بردو|برضه)(\s|$)/gu, " ")
